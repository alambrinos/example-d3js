...
function artistsRecursive1(index_artist, callback) {
    ...
    LAST_FM.artist.getSimilar({
        ...
        success : function(data) {
            if (data) {
                var similarArtists = '';
                var N = data.similarartists.artist.length;
                for (var i = 0; i < N - 1; i++) {
                    similarArtists += data.similarartists.artist[i].name + ',';
                }
                if (N > 0) {
                    similarArtists += data.similarartists.artist[N - 1].name;
                }

                if (DEBUG) console.log('similarArtists(' + ARTISTS[index_artist] + ') == ' + similarArtists);
                
                index_artist++;
                artistsRecursive2(similarArtists, index_artist, 0, callback);    
            }
        },
        error : function(data) {}
    });
...
function artistsRecursive2(similarArtists, index_artist, index_neighbour, callback) {
    ...
    LAST_FM.tasteometer.compare({
        ...
        success : function(data) {
            if (data) {
                if (DEBUG) console.log('compare(user[' + NEIGHBOURS.neighbours.user[index_neighbour].name + '],artists[' + similarArtists + '])');
